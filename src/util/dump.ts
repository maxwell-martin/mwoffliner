  ...
  while ((match = cssUrlRegexp.exec(body))) {
    ...
    if (url && !url.match('^data')) {
      ...
      if (filePathname) {
        ...
        /* Rewrite the CSS */
        const relativePath = articleId ? getRelativeFilePath(articleId, `${config.output.dirs.mediawiki}/${filename}`) : isJs ? `__RELATIVE_FILE_PATH__${config.output.dirs.mediawiki}/${filename}` : filename
  ...
  function hackStartUpModule(jsCode: string) {
    ...
    return replaceCodeByRegex(
      ...
      new Map([
        ...
        [/addScript\(sourceLoadScript\+'\?'\+makeQueryString\(query\),null,packed.list\);/, 'addScript(RLCONF.zimRelativeFilePath+"'+jsPath('"+packed.str+"', config.output.dirs.mediawiki)+'",null,packed.list);'],
  ...
  if (type === 'js') {
    switch (module) {
      case 'startup':
        ...
        break
      case 'mediawiki.base':
        text = replaceCodeByRegex(text, new Map([
    ...
    if (embeddedCss) {
      try {
        ...
        const processedCss = JSON.stringify(cssParts.map((cssPart) => {
